...
def wpm(typed, elapsed):
    ...
    typed_length=len(typed)/5
    return typed_length/elapsed_min
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for i in range(len(valid_words)):
        if valid_words[i]==typed_word:
                return typed_word
        if diff_function(typed_word, valid_words[i],limit)<diff_function(typed_word, valid_words[smallest_diff],limit):
            smallest_diff=i
    if diff_function(typed_word, valid_words[smallest_diff],limit)<=limit:
        return valid_words[smallest_diff]
    else:
        return typed_word
    # END PROBLEM 5
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:  # Fill in the condition
        ...
        return 0
        # END
    if start == "":  # Feel free to remove or add additional cases
        # BEGIN
        return len(goal)
    if goal == "":
        return len(start)
    ...
    if limit ==0:
        ...
        # BEGIN
        return min(add+1, remove+1, substitute+1)
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        subset = []
        for k in range(len(times_per_player[i])-1):
            subset.append(times_per_player[i][k+1]-times_per_player[i][k])
        difference.append(subset)
...
def fastest_words(match):
    ...
    for i in player_indices:
        big_bracket.append([])
    index = 0
    while index < len(get_words(match)):
        min_time = time(match, 0, index)
        min_time_index = 0
        for i in range(len(get_times(match))-1):
            if time(match, i+1, index) < min_time:
                min_time = time(match, i+1, index)
                min_time_index = i+1
        big_bracket[min_time_index].append(word_at(match, index))
        index += 1
    return big_bracket
